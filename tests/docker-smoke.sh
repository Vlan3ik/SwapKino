...
movie_count=$(curl -fsS "$BASE_URL/api/v1/movies?page=1" | python3 -c 'import json,sys; data=json.load(sys.stdin); assert isinstance(data.get("results"), list); print(len(data["results"]))')
