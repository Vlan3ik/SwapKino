...
export function AvatarCropModal({ file, onCancel, onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    const resize = () =>
      setViewport(
        Math.min(380, Math.max(260, viewportRef.current?.clientWidth ?? 360)),
      );
    resize();
  ...
  const scale = natural.width
    ? Math.max(viewport / natural.width, viewport / natural.height) * zoom
    : 1;
  const rendered = {
  ...
  const limit = {
    x: Math.max(0, (rendered.width - viewport) / 2),
    y: Math.max(0, (rendered.height - viewport) / 2),
  };
  ...
  const updateZoom = (next: number) => {
    setZoom(next);
    setOffset((value) => {
      const nextScale = natural.width
        ? Math.max(viewport / natural.width, viewport / natural.height) * next
        : 1;
      const nextRendered = {
      ...
      return {
        x: Math.max(
          -(nextRendered.width - viewport) / 2,
          Math.min((nextRendered.width - viewport) / 2, value.x),
        ),
        y: Math.max(
          -(nextRendered.height - viewport) / 2,
          Math.min((nextRendered.height - viewport) / 2, value.y),
        ),
      };
    });
  };
  const complete = async () => {
    ...
    setSaving(true);
    try {
      onComplete(await cropImage(imageUrl, natural, viewport, scale, offset));
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      <div className="w-full max-w-xl overflow-hidden rounded-3xl border border-white/10 bg-[#15151b] shadow-2xl">
        ...
        <div
          ...
          className="relative mx-auto aspect-square w-full max-w-[380px] cursor-grab touch-none overflow-hidden bg-[#09090b] active:cursor-grabbing"
        >
          ...
          {natural.width === 0 && (
            <div className="pointer-events-none absolute inset-x-0 bottom-4 z-20 text-center text-xs text-white/65">
        ...
        <div className="space-y-4 border-t border-white/10 px-5 py-5">
          ...
          <div className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[.03] p-3">
              ...
              {natural.width > 0 && (
                <img
                  ...
                  style={{
                    left: "50%",
                    top: "50%",
                    width: rendered.width * (80 / viewport),
                    height: rendered.height * (80 / viewport),
                    transform: `translate(-50%, -50%) translate(${offset.x * (80 / viewport)}px, ${offset.y * (80 / viewport)}px)`,
                  }}
                />
              )}
            </div>
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Круг справа показывает итоговый кадр профиля.
              </p>
            </div>
          </div>

          <p className="text-xs text-muted-foreground">
            Потяни изображение мышью или пальцем. В квадрат попадёт только
            выбранная область.
          </p>
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-rating px-4 py-2.5 text-sm font-bold text-black hover:bg-rating/90 disabled:opacity-50"
            >
              <Check className="h-4 w-4" />{" "}
              {saving ? "Готовим…" : "Применить кадр"}
            </button>
          </div>
        </div>
      </div>
...
async function cropImage(
  ...
  const sourceX = Math.max(
    ...
    Math.min(
      ...
      (natural.width - sourceWidth) / 2 - offset.x / scale,
    ),
  );
  const sourceY = Math.max(
    ...
    Math.min(
      ...
      (natural.height - sourceHeight) / 2 - offset.y / scale,
    ),
  );
