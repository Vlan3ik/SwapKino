  ...
  return <><main className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-5">
    ...
    <section className="glass-panel rounded-3xl p-5 sm:p-7">
      ...
      <form onSubmit={saveProfile} className="space-y-4">
        ...
        <div className="flex items-center gap-3"><button type="submit" disabled={busy !== null} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-black hover:bg-rating disabled:opacity-50"><Save className="h-4 w-4" /> Сохранить</button>{profileMessage && <span className="inline-flex items-center gap-1.5 text-sm text-like"><Check className="h-4 w-4" /> {profileMessage}</span>}</div>
      </form>
    </section>
    ...
    <section className="glass-panel rounded-3xl p-5 sm:p-7">
      ...
      <form onSubmit={savePassword} className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3"><button type="button" onClick={() => setShowPasswords(v => !v)} className="inline-flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground">{showPasswords ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />} {showPasswords ? "Скрыть пароли" : "Показать пароли"}</button><button type="submit" disabled={busy !== null || !currentPassword || !newPassword} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-black hover:bg-rating disabled:opacity-50"><KeyRound className="h-4 w-4" /> Изменить пароль</button></div>
