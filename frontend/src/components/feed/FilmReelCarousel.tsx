...
export function FilmReelCarousel() {
  ...
  const visible = reels.map((item, itemIndex) => { let position = itemIndex - index; if (position > reels.length / 2) position -= reels.length; if (position < -reels.length / 2) position += reels.length; return { item, itemIndex, position }; }).filter(({ position }) => Math.abs(position) <= 1);

  return <section className="relative min-h-[calc(100svh-5.5rem)] overflow-hidden rounded-3xl">
    ...
    <motion.div drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.15} onDragEnd={(_: unknown, info: PanInfo) => { if (info.offset.x < -80) go(1); if (info.offset.x > 80) go(-1); }} className="relative h-[calc(100svh-14rem)] min-h-[560px] flex items-center justify-center cursor-grab">
...
function BackgroundCrossfade({ reels, index }: { reels: FilmReel[]; index: number }) {
  useEffect(() => {
    reels.forEach((reel) => { if (reel.coverUrl) { const image = new Image(); image.decoding = "async"; image.src = reel.coverUrl; } });
  }, [reels]);
  const current = reels[index]?.coverUrl;
  const previous = reels[(index - 1 + reels.length) % reels.length]?.coverUrl;
  return <div className="pointer-events-none absolute inset-0 -z-20 overflow-hidden bg-background">
...
function ReelCard({ reel, position, cover, onSide }: { reel: FilmReel; position: number; cover: string | null | undefined; onSide: () => void }) {
  ...
  return <motion.article animate={{ x: position * 390, rotateY: position * -18, scale: center ? 1 : .84, opacity: 1 - distance * .18, z: -distance * 160 }} transition={{ type: "spring", stiffness: 230, damping: 30 }} style={{ transformStyle: "preserve-3d", zIndex: 20 - distance }} onClick={center ? undefined : onSide} className={cn("absolute h-[500px] w-[320px] sm:h-[540px] sm:w-[380px] rounded-3xl overflow-hidden border border-white/10 shadow-cinematic", !center && "cursor-pointer")}>
