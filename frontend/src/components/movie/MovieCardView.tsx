...
export function MovieCardView({
  ...
  const cached = useAppStore((state) =>
    state.movies.find(
      (movie) => movie.id === movieId && (movie.type === "series") === isSeries,
    ),
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      else setError(apiError.message || "Не удалось загрузить карточку");
    } finally {
      setLoading(false);
    }
  }, [movieId, isSeries]);
  ...
  if (notFound)
    return (
      <State
        ...
        text="Возможно, запись была удалена или ссылка устарела."
        onBack={() => router.back()}
      />
    );
  ...
  return (
    <div className="pb-12">
      <section className="relative min-h-[480px] overflow-hidden mb-9">
        {movie.backdropUrl && (
          <img
        ...
        <button
          type="button"
          onClick={() => router.back()}
          className="absolute left-4 sm:left-8 top-4 z-10 rounded-full glass-panel-strong px-4 py-2 text-sm flex items-center gap-2"
        >
          <ArrowLeft className="h-4 w-4" />
          Назад
        </button>
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-28 pb-8 min-h-[480px] flex items-end">
          <div className="flex gap-6 items-end">
            <ArtworkImage
              ...
              alt={movie.title}
              className="hidden sm:grid w-48 aspect-[2/3] rounded-2xl object-cover border border-white/10 shadow-cinematic"
            />
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-4 text-sm text-muted-foreground">
                {movie.rating != null && (
                  <span className="text-rating font-bold">
                ...
                {movie.year && (
                  <span>
                    <Calendar className="inline h-4 w-4" /> {movie.year}
                  </span>
                )}
                {movie.duration && (
                  <span>
      ...
      {error && (
        <div className="mx-auto max-w-7xl px-4 mb-6 rounded-xl border border-skip/30 bg-skip/10 p-4 text-sm">
          Полные данные временно недоступны: {error}{" "}
          <button
            type="button"
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-10">
        <section className="grid lg:grid-cols-3 gap-6">
          {movie.description && (
            <div className="lg:col-span-2">
              <Heading>
                О {movie.type === "series" ? "сериале" : "фильме"}
              </Heading>
        ...
        {movie.trailerYoutubeId && (
          <section>
            ...
            <div className="aspect-video max-w-4xl rounded-2xl overflow-hidden bg-black">
              <iframe
        ...
        {(movie.images.length > 0 || movie.posterUrl || movie.backdropUrl) && (
          <section>
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              <Heading>Кадры и постеры</Heading>
              <Link
                href={`/movie/${movie.id}/images${movie.type === "series" ? "?series=1" : ""}`}
                className="rounded-full border border-white/15 px-4 py-2 text-sm font-semibold hover:bg-white/5"
              >
                Все кадры и постеры
              </Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-w-5xl">
              {[movie.posterUrl, movie.backdropUrl, ...movie.images]
                .filter(
                  (url, index, rows): url is string =>
                    Boolean(url) && rows.indexOf(url) === index,
                )
                .slice(0, 3)
                .map((url, index) => (
                  <Link
                    ...
                    className={cn(
                      ...
                      index === 0 ? "aspect-[2/3]" : "aspect-video",
                    )}
                  >
                    <img
                      src={url}
        ...
        {movie.cast.length > 0 && (
          <section>
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              <Heading>В ролях</Heading>
              <Link
                href={`/movie/${movie.id}/cast${movie.type === "series" ? "?series=1" : ""}`}
                className="rounded-full border border-white/15 px-4 py-2 text-sm font-semibold hover:bg-white/5"
              >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {movie.cast.slice(0, 6).map((person) => (
                <Link
                  ...
                  className="glass-panel rounded-xl overflow-hidden transition hover:-translate-y-1"
                >
                  {person.photoUrl ? (
                    <img
                      src={person.photoUrl}
                    ...
                    <div className="aspect-square grid place-items-center bg-white/5">
...
function Heading({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="text-xl font-bold mb-3 flex gap-2">
      <span className="w-1 rounded-full bg-rating" />
      {children}
    </h2>
  );
}
...
function merge(current: Movie[], incoming: Movie) {
  ...
  const found = current.some((movie) => `${movie.type}:${movie.id}` === key);
  return found
    ? current.map((movie) =>
        `${movie.type}:${movie.id}` === key ? incoming : movie,
      )
    : [...current, incoming];
}
