...
export function MovieCardView({ movieId, isSeries = false }: { movieId: number; isSeries?: boolean }) {
  ...
  const scrollToPlayer = () => document.getElementById("watch")?.scrollIntoView({ behavior: "smooth", block: "start" });
  return <div className="pb-12">
    <section className="relative min-h-[480px] overflow-hidden mb-9">{movie.backdropUrl && <img src={movie.backdropUrl} alt="" className="absolute inset-0 h-full w-full object-cover"/>}<div className="absolute inset-0 bg-gradient-to-t from-background via-background/65 to-background/30"/><button onClick={() => router.back()} className="absolute left-4 sm:left-8 top-4 z-10 rounded-full glass-panel-strong px-4 py-2 text-sm flex items-center gap-2"><ArrowLeft className="h-4 w-4"/>Назад</button>
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-28 pb-8 min-h-[480px] flex items-end"><div className="flex gap-6 items-end"><ArtworkImage src={movie.posterUrl} title={movie.title} fallbackLabel="Постер не загружен" alt={movie.title} className="hidden sm:grid w-48 aspect-[2/3] rounded-2xl object-cover border border-white/10 shadow-cinematic"/><div><div className="flex flex-wrap gap-2 text-[10px] uppercase tracking-widest text-muted-foreground">{movie.type === "series" && <span className="text-rating">Сериал</span>}{movie.genres.map((genre) => <span key={genre}>{genre}</span>)}</div><h1 className="mt-2 text-4xl sm:text-5xl font-bold">{movie.title}</h1>{movie.originalTitle && <p className="mt-1 text-lg italic text-muted-foreground">{movie.originalTitle}</p>}<div className="mt-4 flex flex-wrap gap-4 text-sm text-muted-foreground">{movie.rating != null && <span className="text-rating font-bold"><Star className="inline h-5 w-5 fill-current"/> {movie.rating.toFixed(1)}</span>}{movie.year && <span><Calendar className="inline h-4 w-4"/> {movie.year}</span>}{movie.duration && <span><Clock className="inline h-4 w-4"/> {formatDuration(movie.duration)}</span>}</div>{movie.shortDescription && <p className="mt-4 max-w-2xl text-lg leading-relaxed">{movie.shortDescription}</p>}<div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={scrollToPlayer} className="rounded-full bg-white text-black px-5 py-3 text-sm font-bold flex gap-2 transition hover:bg-rating"><Play className="h-4 w-4 fill-current"/>Смотреть</button><button type="button" aria-pressed={favorite} onClick={(event) => { event.preventDefault(); toggleFavorite(movie.id, isSeries); }} className={cn("rounded-full border px-5 py-3 text-sm font-bold flex gap-2 transition hover:border-like", favorite && "bg-like border-like text-black")}><Heart className="h-4 w-4" fill={favorite ? "currentColor" : "none"}/>{favorite ? "В избранном" : "В избранное"}</button></div></div></div></div>
    </section>
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-10">
      ...
      <MoviePlayer movieId={movie.id} isSeries={isSeries} title={movie.title} />
