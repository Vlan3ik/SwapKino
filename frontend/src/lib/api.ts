...
export interface AdminFeature {
  featureType: "genre" | "keyword";
  tmdbFeatureId: number;
  weight: number;
  mode: "required" | "preferred" | "excluded";
}
export interface AdminReference {
  tmdbId: number;
  weight: number;
}
export interface AdminFilmstrip {
  id?: string;
  name: string;
  slug: string;
  isSeries: boolean;
...
function apiError(
  ...
  const message = apiErrorMessage(status, data);
...
function apiErrorMessage(status: number, data: unknown): string {
  const errors =
    ...
    Array.isArray((data as { errors: unknown }).errors)
      ? (data as { errors: Array<{ code?: string; description?: string }> })
          .errors
      : null;
  ...
  if (
    ...
    return data
      .map((item) => String((item as { description: unknown }).description))
      .join(" ");
...
export const api = {
  ...
  movies: (query: CatalogQuery | number = {}) => {
    const normalized: CatalogQuery =
      typeof query === "number"
        ? { cursor: query > 1 ? String(query) : undefined }
        : query;
    ...
    if (normalized.genreIds?.length)
      params.set("genreIds", normalized.genreIds.join(","));
    if (normalized.minRating != null)
      params.set("minRating", String(normalized.minRating));
    ...
    if (normalized.isSeries != null)
      params.set("isSeries", String(normalized.isSeries));
    if (normalized.sort)
      params.set(
        "sort",
        (
  ...
  favorites: (query: LibraryQuery = {}) =>
    request<ApiLibraryPage>(`/favorites?${libraryParams(query)}`, {
      signal: query.signal,
    }),
  ratings: (query: LibraryQuery = {}) =>
    request<ApiLibraryPage>(`/ratings?${libraryParams(query)}`, {
      signal: query.signal,
    }),
  reels: () =>
    request<{ items?: ApiReel[]; results?: ApiReel[] } | ApiReel[]>(
      "/filmstrips",
    ),
  adminFilmstrips: () => request<AdminFilmstrip[]>("/admin/filmstrips"),
  adminCreateFilmstrip: (payload: {
    name: string;
    isSeries: boolean;
    features: AdminFeature[];
    references: AdminReference[];
  }) =>
    request<AdminFilmstrip>("/admin/filmstrips", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  adminUpdateFilmstrip: (
    id: string,
    payload: {
      name: string;
      isSeries: boolean;
    ...
    request<AdminFilmstrip>(`/admin/filmstrips/${id}`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    }),
  adminSetFilmstripStatus: (id: string, status: "publish" | "unpublish") =>
    request<AdminFilmstrip>(`/admin/filmstrips/${id}/${status}`, {
      method: "POST",
    }),
  adminDeleteFilmstrip: (id: string) =>
    request<void>(`/admin/filmstrips/${id}`, { method: "DELETE" }),
  adminPreviewFilmstrip: (id: string) =>
    request<{
      strictCount: number;
      relaxedCount: number;
      referenceRecommendations: number;
      referenceSimilar: number;
      uniqueCandidates: number;
      duplicateCount: number;
      coverAvailable: boolean;
      warning?: string | null;
    }>(`/admin/filmstrips/${id}/preview`, { method: "POST" }),
  adminGenres: (isSeries = false) =>
    request<{ genres?: Array<{ id: number; name: string }> }>(
      `/admin/tmdb/genres?isSeries=${isSeries}`,
    ),
  adminKeywords: (q: string) =>
    request<{ results?: Array<{ id: number; name: string }> }>(
      `/admin/tmdb/keywords?q=${encodeURIComponent(q)}`,
    ),
  adminSearchMovies: (q: string, isSeries = false) =>
    request<{
      results?: Array<{
        id: number;
        title?: string;
    ...
    }>(`/admin/tmdb/search?q=${encodeURIComponent(q)}&isSeries=${isSeries}`),
  adminSuggestions: (
    name: string,
    referenceIds: number[],
    featureIds: number[],
    ...
    request<{
      movies?: Array<{ id: number; name: string; poster_path?: string | null }>;
      genres?: Array<{ id: number; name: string }>;
      keywords?: Array<{ id: number; name: string }>;
    }>(
      `/admin/tmdb/suggestions?name=${encodeURIComponent(name)}&referenceIds=${referenceIds.join(",")}&featureIds=${featureIds.join(",")}&genreIds=${genreIds.join(",")}&isSeries=${isSeries}`,
    ),
  reelFeed: (
    slug: string,
    cursor?: string | null,
    ...
    return request<ReelFeed>(
      `/filmstrips/${encodeURIComponent(slug)}/feed?${params}`,
    );
  },
  action: (payload: {
    tmdbId: number;
    ...
    request<{ id: string; duplicate: boolean }>("/actions", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  ...
  importResume: (id: string) =>
    request<ImportStatus>(`/imports/${id}/resume`, { method: "POST" }),
  importCancel: (id: string) =>
    request<ImportStatus>(`/imports/${id}/cancel`, { method: "POST" }),
...
export function mapApiMovie(movie: ApiMovie): Movie {
  ...
  const genreItems = mapGenres(movie, payload);
  const { cast, directors, writers } = mapPeople(movie, payload);
  const videos =
    typeof payload.videos === "object" && payload.videos
      ? (payload.videos as { results?: unknown[] })
      : {};
  const videoRows =
    movie.trailers ?? (Array.isArray(videos.results) ? videos.results : []);
  const trailer = videoRows
    .map((video) => video as { site?: string; type?: string; key?: string })
...
function mapGenres(movie: ApiMovie, payload: Record<string, unknown>): Genre[] {
  const canonical = Array.isArray(movie.genres)
    ? movie.genres.map((genre) => ({
        id: genre.id,
        slug: genre.slug || String(genre.id),
        name: genre.name,
      }))
    : [];
  ...
  if (Array.isArray(payload.genres)) {
    raw = payload.genres
      .map((genre) => typeof genre === "object" && genre && "name" in genre ? String(genre.name) : "")
      .filter(Boolean);
  } else if (Array.isArray(payload.genre_ids)) {
    raw = payload.genre_ids.map((id) => genreNames[Number(id)]).filter(Boolean);
  }
...
function mapPeople(
  ...
  const credits =
    typeof payload.credits === "object" && payload.credits
      ? (payload.credits as { cast?: unknown[]; crew?: unknown[] })
      : {};
  const mapPerson = (person: ApiPerson, fallback: string): Person => ({
    id: person.id ?? 0,
    name: person.name ?? "",
    role: person.role || person.character || fallback,
    photoUrl:
      person.photoUrl ??
      (person.profile_path
  ...
  const cast =
    movie.cast?.map((person) => mapPerson(person, "Актёр")) ??
    mapCast(credits.cast);
  const crew = movie.crew ?? (Array.isArray(credits.crew) ? credits.crew : []);
  const people = crew.map(
    (person) => person as { id?: number; name?: string; job?: string },
  );
  const directors =
    movie.directors?.map((person) => mapPerson(person, "Режиссёр")) ??
    people
  ...
  const writers =
    movie.writers?.map((person) => mapPerson(person, "Сценарий")) ??
    people
      .filter(
  ...
  const trailer = videoRows
    .map((video) => video as { site?: string; type?: string; key?: string })
    .find((video) => video.site === "YouTube" && video.type === "Trailer");
  return { cast, directors, writers, trailer };
}

function mapCast(values: unknown[] | undefined): Person[] {
  if (!Array.isArray(values)) return [];
  return values.slice(0, 8).map((person) => {
    const item = person as {
      id?: number;
      name?: string;
      character?: string;
      profile_path?: string;
    ...
    return {
      id: item.id ?? 0,
      name: item.name ?? "",
      role: item.character ?? "Актёр",
      photoUrl: item.profile_path
...
function mapImages(values: unknown[] | undefined): string[] {
  if (!Array.isArray(values)) return [];
  return values
    .map((image) => {
      if (typeof image === "string") return image;
      if (typeof image === "object" && image && "file_path" in image)
        return `https://image.tmdb.org/t/p/original${String((image as { file_path: unknown }).file_path)}`;
      return null;
    })
