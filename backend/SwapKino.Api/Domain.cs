...
public sealed class RecommendationImpression { public Guid Id { get; set; } = Guid.NewGuid(); public Guid? UserId { get; set; } public int TmdbId { get; set; } public bool IsSeries { get; set; } public string ThemeId { get; set; } = ""; public int Position { get; set; } public string Reason { get; set; } = ""; public string FeedItemType { get; set; } = "movie"; public string RankerVersion { get; set; } = "local-v1"; public int ProfileVersion { get; set; } public string? SessionId { get; set; } public DateTime ShownAt { get; set; } = DateTime.UtcNow; }
...
// Product-owned recommendation state. TMDB content is deliberately represented
// only by (media type, TMDB id); no title, artwork or payload belongs here.
public enum FilmstripFeatureType { genre, keyword }
public enum FilmstripFeatureMode { required, preferred, excluded }
public sealed class Filmstrip
{
    public Guid Id { get; set; } = Guid.NewGuid(); public string Name { get; set; } = "";
    public string Slug { get; set; } = ""; public bool IsSeries { get; set; }
    public string Status { get; set; } = "draft"; public int ConfigVersion { get; set; } = 1;
    public DateTime CreatedAt { get; set; } = DateTime.UtcNow; public DateTime UpdatedAt { get; set; } = DateTime.UtcNow;
    public ICollection<FilmstripFeature> Features { get; set; } = []; public ICollection<FilmstripReference> References { get; set; } = [];
}
public sealed class FilmstripFeature
{
    public Guid FilmstripId { get; set; } public FilmstripFeatureType FeatureType { get; set; }
    public int TmdbFeatureId { get; set; } public double Weight { get; set; } = 1;
    public FilmstripFeatureMode Mode { get; set; } = FilmstripFeatureMode.preferred; [JsonIgnore] public Filmstrip Filmstrip { get; set; } = null!;
}
public sealed class FilmstripReference
{
    public Guid FilmstripId { get; set; } public int TmdbId { get; set; } public bool IsSeries { get; set; }
    public double Weight { get; set; } = 1; [JsonIgnore] public Filmstrip Filmstrip { get; set; } = null!;
}
public sealed class KeywordAlias { public int TmdbKeywordId { get; set; } public string RuName { get; set; } = ""; public string OriginalName { get; set; } = ""; }
public sealed class UserTasteFeature
{
    public Guid UserId { get; set; } public FilmstripFeatureType FeatureType { get; set; }
    public int TmdbFeatureId { get; set; } public double Weight { get; set; }
    public double Confidence { get; set; } public int ObservationCount { get; set; }
    public DateTime UpdatedAt { get; set; } = DateTime.UtcNow;
}
...
public sealed class SwapKinoDbContext(DbContextOptions<SwapKinoDbContext> options) : IdentityDbContext<User, IdentityRole<Guid>, Guid>(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        b.Ignore<Movie>(); b.Ignore<Genre>(); b.Ignore<MovieGenre>(); b.Ignore<Keyword>(); b.Ignore<MovieKeyword>(); b.Ignore<MoviePerson>(); b.Ignore<MovieThemeMembership>(); b.Ignore<CatalogSyncState>();
        b.Entity<Filmstrip>().HasKey(x => x.Id); b.Entity<Filmstrip>().HasIndex(x => x.Slug).IsUnique();
        b.Entity<FilmstripFeature>().HasKey(x => new { x.FilmstripId, x.FeatureType, x.TmdbFeatureId });
        b.Entity<FilmstripFeature>().HasOne(x => x.Filmstrip).WithMany(x => x.Features).HasForeignKey(x => x.FilmstripId).OnDelete(DeleteBehavior.Cascade);
        b.Entity<FilmstripReference>().HasKey(x => new { x.FilmstripId, x.TmdbId, x.IsSeries });
        b.Entity<FilmstripReference>().HasOne(x => x.Filmstrip).WithMany(x => x.References).HasForeignKey(x => x.FilmstripId).OnDelete(DeleteBehavior.Cascade);
