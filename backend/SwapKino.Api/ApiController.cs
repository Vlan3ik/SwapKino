...
public sealed class ApiController(SwapKinoDbContext db, UserManager<User> users, IConfiguration config, TmdbClient tmdb) : ControllerBase
{
    ...
    [HttpPost("auth/login")]
    public async Task<IActionResult> Login(LoginRequest request) { var u=await users.FindByEmailAsync(request.Email); if(u is null||!await users.CheckPasswordAsync(u,request.Password))return Unauthorized(new{message="Invalid email or password"}); return Ok(new{accessToken=Token(u),user=new{id=u.Id,email=u.Email,displayName=u.DisplayName}}); }
    [HttpGet("movies")]
    [AllowAnonymous]
    ...
    [HttpGet("library")][Authorize] public async Task<IActionResult> Library() => Ok(new{items=await db.UserActions.Where(x=>x.UserId==UserId).OrderByDescending(x=>x.CreatedAt).Select(x=>new{id=x.Id,tmdbId=x.TmdbId,action=x.ActionType,value=x.Value,createdAt=x.CreatedAt}).ToListAsync()});
